...
def test_performance_indices_setup(config_dict, model="EC-Earth3", ensemble="r1i1p1f1"):
    ...
    try:
        ...
        pi = PerformanceIndices(
            expname,
            year1,
            year1,  # Just one year for quick test
            config=config_dict,
            model=model,
            ensemble=ensemble,
            climatology=refclim,
...
def check_dependencies():
    ...
    for module_name in ("ecmean", "xesmf", "smmregrid"):
...
def run_performance_indices_with_tool(model, ensemble, config_dict, tool, log_level="info"):
    ...
    try:
        ...
        if tool == "CDO":
            print("    Adding delay to reduce resource conflicts...")
            time.sleep(2.0)
    ...
    if esmf_perf and cdo_perf:
        comparison["performance"] = {
            ...
            "speed_comparison": {
                ...
                if cdo_perf["execution_time"] > 0
                else 0.0,
                "time_difference": round(float(abs(esmf_perf["execution_time"] - cdo_perf["execution_time"])), 2),
                "relative_difference": round(
                    float(abs(esmf_perf["execution_time"] - cdo_perf["execution_time"]))
                    / float(max(esmf_perf["execution_time"], cdo_perf["execution_time"])),
                    3,
                )
                if max(esmf_perf["execution_time"], cdo_perf["execution_time"]) > 0
                else 0.0,
            },
        }
...
def main():
    ...
    parser.add_argument("--test-single", action="store_true", help="Test with single model only (EC-Earth3)")
    parser.add_argument("--config", default="../config_CMIP6_esgpull.yml", help="Configuration file to use")
    parser.add_argument("--debug", action="store_true", help="Enable debug mode with more verbose output")
    parser.add_argument("--test-setup-only", action="store_true", help="Only test the setup, do not run full comparison")
    ...
    for i, model in enumerate(models_to_test, 1):
        ...
        if esmf_result["status"] == "success" and cdo_result["status"] == "success":
            ...
            comparison = {
                ...
                "performance": {"esmf": esmf_result.get("performance"), "cdo": cdo_result.get("performance")},
            }
                ...
                if "esmf" in perf and "cdo" in perf:
                    if perf["esmf"] and perf["cdo"]:
                        ...
                        if "speed_comparison" in perf:
                            ...
                            print(
                                f"  ⏱️  Performance: {faster_method} faster by {speedup}x "
    ...
    if significant_models:
        ...
        for model in significant_models:
            print(
                f"  {model['model']}: {model['significant_count']}/{model['total_count']} "
    ...
    if args.test_single:
        print("  2. Run full test: python cdo-esmf-comparison.py")
    print("  3. Check the detailed report and plots in the analysis output")
    print()
