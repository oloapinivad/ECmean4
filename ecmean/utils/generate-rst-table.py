...
def format_rst_table(data, table_name="Data used in climatology"):
    ...
    has_cmip6_models = any(isinstance(info, dict) and info.get("cmip6", {}).get("nmodels") for info in data.values())
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Generate RST table from climatology/reference YAML file.")
    parser.add_argument("yaml_file", type=str, help="Path to the climatology/reference YAML file")
    parser.add_argument("--output", type=str, help="Output file (default: print to terminal)")
