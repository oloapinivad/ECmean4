...
for var in variables:
    ...
    if "outvalue" in info[var].keys():
        ...
        if isinstance(info[var]["dir"], list):
            temp_list = [
                glob(expand_filedata(ss, var, info)) for ss in info[var]["dir"]
            ]
        ...
        xfield = xr.open_mfdataset(
            filedata, chunks="auto", preprocess=xr_preproc, engine="netcdf4"
        )
        ...
        if real_year1 != year1:
            logging.warning(
                "Initial year different from what expected: " + str(real_year1)
            )
        if real_year2 != year2:
            logging.warning(
                "Final year different from what expected: " + str(real_year2)
            )
        ...
        print("Time averages...")
        gfield1 = (
            cfield.resample(time="AS", skipna=nanskipper)
            .mean("time", skipna=nanskipper)
            .load()
        )
        gfield2 = (
            cfield.resample(time="Q-NOV", skipna=nanskipper)
            .mean("time", skipna=nanskipper)
            .load()
        )
        ...
        for season in ["ALL", "DJF", "MAM", "JJA", "SON"]:
            ...
            for region in ["Global", "North Midlat", "Tropical", "South Midlat"]:
                ...
                if season == "ALL" and region == "Global":
                    logging.warning(
                        "{} {} {} mean is: {:.2f} +- {:.2f}".format(
                            var, season, region, omean, ostd
                        )
                    )
                else:
                    logging.info(
