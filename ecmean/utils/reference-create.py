...
__author__ = (
    "Paolo Davini (paolo.davini@cnr.it), Feb 2023. Modified by Marianna Albanese (marianna.albanese@polito.it), Jan 2026."
)
...
from ecmean.utils.utils import timeframe_years, expand_filedata, select_time_period, parse_create_args, select_time_data
        ...
        if "outvalue" in info[var]:
            ...
            if var in FIXED_ICE_VARS:
                ...
                if "south" in var:
                    ...
                    raise ValueError(
                        f"Variable {var} is in FIXED_ICE_VARS but does not contain 'north' or 'south' in its name."
                    )
            ...
            xfield = xr.open_mfdataset(
                filedata, chunks="auto", preprocess=xr_preproc, engine="netcdf4", join="outer", data_vars="all"
            )
