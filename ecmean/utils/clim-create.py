...
from ecmean.utils.utils import (
    check_histogram,
    mask_from_field,
    variance_threshold,
    variance_fraction,
...
# setup logging
logging.basicConfig(
    level=logging.INFO, format="%(asctime)s | %(name)s | %(levelname)8s -> %(message)s", datefmt="%Y-%m-%d %H:%M:%S"
)

# variable list
variables = ["tas", "pr", "net_sfc", "tauu", "tauv", "psl", "ua", "va", "ta", "hus", "tos", "sos", "siconc"]
# variables = ['tas']
# variables.reverse()
...
def main(climdata="EC26", timeframe="HIST", machine="wilma", do_figures=False, overwrite=False, method="sigma"):
    ...
    for var in variables:
        ...
        filedata = str(os.path.expandvars(info[var]["dir"])).format(
            datadir=datadir, archivedir=archivedir, dataset=info[var]["dataset"], varname=info[var]["varname"]
        )
        ...
        if not overwrite:
            files_to_check = get_climatology_files(tgtdir, var, info[var]["dataset"], GRID, real_year1, real_year2).values()
        ...
        for season in ["ALL", "DJF", "MAM", "JJA", "SON"]:
            ...
            if do_figures:
                ...
                os.makedirs(os.path.join(figdir, var), exist_ok=True)
                file = os.path.join(figdir, var, figname)
                check_histogram(ovar, fvar, file, method=method, center=center, low=low, high=high)
        ...
        climatology_data = [full_variance, full_mean, season_variance, season_mean]

        climatology_files = get_climatology_files(tgtdir, var, info[var]["dataset"], GRID, real_year1, real_year2)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--figures", action="store_true", default=False, help="Generate diagnostic figures (default: False)")
    parser.add_argument(
        "--method",
        type=str,
        default=METHOD,
